        ...
        <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid gap-6 mb-6">
                <div>
                    ...
                    <input type="text" id="title" name="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Post title" required value="{{ old('title', $post->title) }}">
                </div>
                <div>
                ...
                <div>
                    ...
                    @if ($post->image)
                        <div class="mt-4">
                            <span class="block text-sm font-medium text-gray-900 dark:text-white mb-2">Current Image:</span>
                            <img src="{{ asset('storage/' . $post->image) }}" alt="{{ $post->title }}" class="w-48 h-auto rounded-lg">
                        </div>
                    @endif
                </div>
            </div>
